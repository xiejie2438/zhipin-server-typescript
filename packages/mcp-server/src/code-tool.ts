...
import { ClientOptions } from 'zhipin-server';

const prompt = `Runs JavaScript code to interact with the Zhipin Server API.
...
const remoteStainlessHandler = async ({
  ...
  const localClientEnvs = {
    ZHIPIN_SERVER_API_KEY: readEnv('ZHIPIN_SERVER_API_KEY') ?? client.apiKey ?? undefined,
    ZHIPIN_SERVER_BASE_URL:
      readEnv('ZHIPIN_SERVER_BASE_URL') ?? readEnv('ZHIPIN_SERVER_ENVIRONMENT') ?
        undefined
  ...
  const res = await fetch(codeModeEndpoint, {
    ...
    body: JSON.stringify({
      project_name: 'zhipin-server',
      code,
      intent,
      client_opts: { environment: (readEnv('ZHIPIN_SERVER_ENVIRONMENT') || undefined) as any },
...
const localDenoHandler = async ({
  ...
  try {
    const sdkPkgName = 'zhipin-server';
