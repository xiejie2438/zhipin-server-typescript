...
export async function initMcpServer(params: {
  ...
  let _client: ZhipinServer | undefined;
  let _clientError: Error | undefined;
  let _logLevel: 'debug' | 'info' | 'warn' | 'error' | 'off' | undefined;

  const getClient = (): ZhipinServer => {
    if (_clientError) throw _clientError;
    if (!_client) {
      try {
        _client = new ZhipinServer({
          ...{ environment: (readEnv('ZHIPIN_SERVER_ENVIRONMENT') || undefined) as any },
          logger,
